...
It specifies the main folder and file locations for our notebook. Each
of these is described by a `comment` field.
...
Finally, **`.here`** is a sentinel file for the here package. It
indicates where the project root is located. **`R/functions.R`** is an
(as-yet empty) R script that is `source()`-ed at the start of
`_targets.R`.
...
### Finer point 1.5: Despite the timestamp, image dependencies are tracked
...
"notebook/book/2022-02-22-hello-world.md". Its plot are also tracked as
byproducts of the entry:
...
This move is what allows image dependencies (see last point) to be
inferred: If an image file is created as a result of knitting an .Rmd
file, we can associate it with .md file.
